...
export default class EditarComponent {
  ...
  constructor(private fb: FormBuilder,private location: Location,public alumnos: ProyectoapiService, private router:Router){}
  ...
  ngOnInit(){
    ...
    this.alumnos.getAlumno(parseInt(this.tem[3])).subscribe(
      {
        next: response=>{
 ...
 modificar(){
  this.alumnos.modificarAlumno(this.tem[3],this.regAlumno).subscribe({
    next:()=>console.log(),
